...
async function cleanupDemoData() {
  const DEMO_IDS = ["demo-user-id", "test-factory-user", "test-user-gifts", "test-agent-room-czMxDC"];
  for (const demoId of DEMO_IDS) {
    const demoTokens = await db.select().from(apiTokens).where(eq(apiTokens.createdById, demoId));
    if (demoTokens.length > 0) {
      await db.delete(apiTokens).where(eq(apiTokens.createdById, demoId));
      console.log(`Cleaned up ${demoTokens.length} demo API tokens from ${demoId}`);
    }
    const demoLogs = await db.select().from(auditLogs).where(eq(auditLogs.userId, demoId));
    if (demoLogs.length > 0) {
      await db.delete(auditLogs).where(eq(auditLogs.userId, demoId));
      console.log(`Cleaned up ${demoLogs.length} demo audit logs from ${demoId}`);
    }
  }
}
