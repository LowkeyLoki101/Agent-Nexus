...
export async function registerRoutes(
  ...
  app.post("/api/topics/:id/messages", isAuthenticated, async (req, res) => {
    try {
      ...
      if (req.body.authorType === "human" || !req.body.authorType) {
        setTimeout(async () => {
          try {
            const { triggerImmediateReply } = await import("./agentDaemon");
            await triggerImmediateReply(req.params.id, userId);
          } catch (e: any) {
            console.error("[BoardNudge] Failed to trigger immediate reply:", e.message);
          }
        }, 2000);
      }
    } catch (error) {
...
Be concise and helpful. Use factory/production metaphors when appropriate.`;
  ...
  // ==================== LineCutterz Product Catalog ====================

  app.get("/api/linecutterz/products", async (req, res) => {
    try {
      const limit = Math.min(parseInt(req.query.limit as string) || 50, 100);
      const offset = parseInt(req.query.offset as string) || 0;
      const products = await storage.getLinecuterzProducts(limit, offset);
      const total = await storage.getLinecuterzProductCount();
  ...
  app.get("/api/linecutterz/products/:id", async (req, res) => {
    try {
      const product = await storage.getLinecuterzProduct(req.params.id);
      if (!product) return res.status(404).json({ message: "Product not found" });
      res.json(product);
    } catch (error) {
      res.status(500).json({ message: "Failed to fetch product" });
  ...
  app.use("/linecutterz-images", express.static(
    path.join(process.cwd(), "attached_assets/Emergent Bridge/linecutterz_products/images"),
    { maxAge: "7d" }
  ));

  // ==================== LineCutterz Storefront Checkout ====================

  app.post("/api/linecutterz/checkout", isAuthenticated, async (req, res) => {
    try {
      const { listingId } = req.body;
      if (!listingId) return res.status(400).json({ error: "listingId required" });

      const listing = await storage.getStorefrontListingById(listingId);
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        line_items: [{
          price_data: {
            ...
            product_data: {
              name: listing.title,
              description: listing.description || "AI-generated marketing content for LineCutterz products",
            },
        ...
        mode: "payment",
        success_url: `${req.headers.origin || "https://pocket-factory.replit.app"}/linecutterz?purchased=${listingId}`,
        cancel_url: `${req.headers.origin || "https://pocket-factory.replit.app"}/linecutterz`,
        metadata: { listingId, type: "linecutterz_marketing" },
      });
  ...
  app.get("/api/linecutterz/download/:listingId", isAuthenticated, async (req, res) => {
    try {
      const listing = await storage.getStorefrontListingById(req.params.listingId);
      if (!listing) return res.status(404).json({ message: "Listing not found" });

      const userId = getUserId(req as AuthenticatedRequest);
      const purchases = await db.select().from(
        (await import("@shared/schema")).storefrontPurchases
      ).where(
        sql`listing_id = ${req.params.listingId} AND buyer_email = (SELECT email FROM users WHERE id = ${userId}) AND status = 'completed'`
      );

      const user = await storage.getUser(userId);
      const isAdmin = user && (user as any).isAdmin;

      if (!isAdmin && purchases.length === 0) {
        return res.status(403).json({ message: "Purchase required to download" });
      }

      res.json({
        title: listing.title,
        content: listing.downloadContent || listing.previewContent,
