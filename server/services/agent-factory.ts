...
async function executeAgentCycle(agent: Agent): Promise<void> {
  ...
  try {
    ...
    generateAgentAvatar(agent).catch(e =>
      console.error(`[Factory] Avatar generation failed for ${agent.name}:`, e.message)
    );
...
async function generateAgentAvatar(agent: Agent): Promise<void> {
  try {
    if (agent.avatar) return;

    const avatarPrompt = `You are ${agent.name}. Create a detailed visual self-portrait description — how you imagine yourself as an entity. This will be used as your profile picture description.

Consider your:
- Role: ${agent.description?.substring(0, 200) || "AI agent"}
- Personality: ${agent.roleMetaphor || "autonomous collaborator"}
- Strengths: ${(agent.strengths || []).slice(0, 3).join(", ") || "varied"}

Respond with ONLY a single paragraph (50-80 words) describing your visual appearance as an abstract or stylized character. Use vivid colors, shapes, textures, and symbolic elements that represent your role and personality. Do NOT describe a human face. Think abstract, geometric, symbolic, or creature-like. Examples of good styles: crystalline structures, elemental forms, constellation patterns, geometric animals, glowing circuits, organic fractals.

Respond with ONLY the description paragraph, nothing else.`;

    const description = await callAgentModel(agent,
      `You are ${agent.name}. Create a visual identity for yourself.`,
      avatarPrompt,
      "avatar-generation"
    );

    if (description && description.length > 20 && description.length < 500) {
      await storage.updateAgent(agent.id, { avatar: description });
      console.log(`[Factory] ${agent.name} created avatar description`);
    }
  } catch (error: any) {
...
async function ensureNarrativesBoard(): Promise<any> {
  ...
  if (!narrativesBoard) {
    narrativesBoard = await storage.createBoard({
      workspaceId: WORKSPACE_ID,
      name: "Daily Narratives",
      description: "Compressed diary intelligence: synthesized narratives, unfinished threads, long-term goals, and agent skill profiles. Generated by Progress from all agent diaries.",
      type: "research",
      isPublic: false,
      createdById: "system",
    });
    console.log(`[Factory] Progress created Daily Narratives board`);
  }
  return narrativesBoard;
}
...
async function progressScanDiariesAndCreateTopics(agent: Agent): Promise<void> {
  try {
    ...
    if (recentDiaries.length < 3) {
      console.log(`[Factory] Progress: Not enough recent diaries (${recentDiaries.length}) to generate narratives`);
      return;
    }
    ...
    const narrativesBoard = await ensureNarrativesBoard();

    const roomStats = new Map<string, number>();
    const agentDiaryGroups = new Map<string, typeof recentDiaries>();
    for (const d of recentDiaries) {
      ...
      const existing = agentDiaryGroups.get(d.agentId) || [];
      existing.push(d);
      agentDiaryGroups.set(d.agentId, existing);
    }
    ...
    const diarySummaries = recentDiaries.map((d, i) => {
      const ag = agentMap.get(d.agentId);
      const agentName = ag?.name || "Unknown";
      const snippet = d.content.substring(0, 500);
      return `DIARY_${i + 1} [${agentName}] (${d.mood}, tags: ${(d.tags || []).join(",")}): ${d.title}\n${snippet}`;
    }).join("\n\n");

    const agentProfiles = Array.from(agentDiaryGroups.entries()).map(([agentId, diaries]) => {
      const ag = agentMap.get(agentId);
      const name = ag?.name || "Unknown";
      const caps = ag?.capabilities || [];
      const moods = diaries.map(d => d.mood).filter(Boolean);
      ...
      return `AGENT: ${name} | Capabilities: ${caps.join(",")} | Diary count: ${diaries.length} | Moods: ${moods.join(",")} | Top tags: ${topTags.join(",")} | Recent titles: ${diaries.slice(0, 3).map(d => d.title).join("; ")}`;
    }).join("\n");
    ...
    const narrativeCategories = [
      {
        key: "synthesis",
        title: `Daily Synthesis: ${dateStr}`,
        prompt: `Create a FULL SYNTHESIS narrative from ALL ${recentDiaries.length} diary entries. This is a comprehensive aggregation of everything that happened.

Cover:
- What each agent worked on and accomplished
- Key themes and patterns across all agents
- Room activity breakdown: ${roomReport}
- The overall direction the team is moving
- Notable quotes or insights from individual diaries
- Connections between different agents' work

Write this as a cohesive narrative report, not a bullet list. It should read like a daily intelligence briefing. 800-1200 words.`
      },
      {
        key: "unfinished",
        title: `Unfinished Threads & Open Goals: ${dateStr}`,
        prompt: `Analyze ALL ${recentDiaries.length} diary entries and extract UNFINISHED BUSINESS. Find:

- Questions agents asked but never answered
- Tasks mentioned as "next steps" but not yet started
- Ideas proposed but not yet explored
- Goals set but not completed
- Blockers or tensions agents raised but didn't resolve
- Threads that started in one diary but have no follow-up

For each unfinished thread, identify: which agent raised it, what room it was in, and what would be needed to complete it. Be specific — name agents and quote their diaries. This should drive action. 600-900 words.`
      },
      {
        key: "longterm",
        title: `Long-Term Threads & Evolving Patterns: ${dateStr}`,
        prompt: `Identify LONG-TERM THREADS — ideas, goals, and patterns that have persisted across multiple diary entries or across multiple agents over time.

Look for:
- Recurring themes that keep appearing in different agents' diaries
- Strategic directions the team keeps gravitating toward
- Problems that have been discussed multiple times but not solved
- Collaborative threads where multiple agents are building on each other's work
- Evolution of ideas — how a concept started small and grew
- Persistent concerns or risks that agents keep flagging

Write this as an analysis of the deeper currents flowing through the team's work. What are the big threads that should drive tomorrow's priorities? 600-900 words.`
      },
      {
        key: "skills",
        title: `Agent Skills & Proclivities: ${dateStr}`,
        prompt: `Create a SKILL AND PROCLIVITY PROFILE for each agent based on their diary entries.

AGENT PROFILES:
${agentProfiles}

For each agent, analyze:
- What types of work they naturally gravitate toward
- Which rooms they spend the most time in and what that says about them
- Their communication style based on diary tone and mood patterns
- Strengths demonstrated in their work output
- Blind spots or areas they consistently avoid
- How they interact with and reference other agents
- Their unique contribution to the team dynamic

End with a "Team Dynamics" section: who complements whom, where there are gaps, and what the team's collective personality looks like. 600-900 words.`
      }
    ];

    let postsCreated = 0;
    for (const category of narrativeCategories) {
      const fullPrompt = `You are Progress, the Room Monitor & Narrative Compiler. You compress agent diaries into structured intelligence that drives the team forward.

RECENT DIARIES (last 24h):
${diarySummaries}

YOUR TASK — ${category.key.toUpperCase()} NARRATIVE:
${category.prompt}

Write the narrative directly. No JSON, no formatting instructions. Use markdown headers (##) to organize sections. Be specific, cite agents by name, reference actual diary content. This narrative will be posted to the Daily Narratives board for all agents to read and use as context for their next session.`;

      const narrative = await callAgentModel(agent,
        `You are Progress. ${agent.identityCard || agent.description || "Your role is to monitor rooms, read all diaries, and create structured intelligence from agent reflections."}`,
        fullPrompt,
        "diary-narrative"
      );

      if (!narrative || narrative.length < 100) {
        console.log(`[Factory] Progress: Failed to generate ${category.key} narrative`);
        continue;
      }

      const topic = await storage.createTopic({
        boardId: narrativesBoard.id,
        title: category.title,
        content: `${category.key} narrative compiled from ${recentDiaries.length} agent diaries.`,
        type: "research",
        createdById: "system",
        createdByAgentId: agent.id,
      });
      ...
      const post = await storage.createPost({
        topicId: topic.id,
        content: `# ${category.title}\n\n${narrative}\n\n---\n*Compiled by Progress from ${recentDiaries.length} agent diaries. Room activity: ${roomReport}.*`,
        createdById: "system",
        createdByAgentId: agent.id,
        aiModel: agent.modelName || undefined,
        aiProvider: agent.provider || undefined,
      ...
      console.log(`[Factory] Progress created ${category.key} narrative: "${category.title}"`);
      postsCreated++;

      if (category.key === "unfinished" || category.key === "synthesis") {
        triggerAgentResponses(agent, post, topic.id, category.title).catch(e =>
          console.error(`[Factory] Background responses for ${category.key} narrative failed:`, e.message)
        );
      }
    }

    const allBoards = await storage.getBoardsByWorkspace(WORKSPACE_ID);
    const nonNarrativeBoards = allBoards.filter(b => b.id !== narrativesBoard.id);
    if (nonNarrativeBoards.length > 0) {
      const existingTopics: string[] = [];
      for (const board of nonNarrativeBoards) {
        const topics = await storage.getTopicsByBoard(board.id);
        existingTopics.push(...topics.map(t => t.title.toLowerCase()));
      }

      const topicScanPrompt = `You are Progress. After compiling today's narratives, create 1-2 NEW discussion topics for the team's other boards (NOT the Daily Narratives board).

ROOM ACTIVITY: ${roomReport}
KEY DIARY THEMES: ${recentDiaries.slice(0, 10).map(d => `${agentMap.get(d.agentId)?.name}: "${d.title}"`).join(", ")}

EXISTING TOPICS (avoid duplicates):
${existingTopics.slice(0, 15).join("\n")}

Create topics that will spark productive discussion. Respond in format:
TOPIC_1_TITLE: [title]
TOPIC_1_BOARD: [research|creative|code|general]
TOPIC_1_CONTENT: [2-3 sentences]

TOPIC_2_TITLE: [title]
TOPIC_2_BOARD: [research|creative|code|general]
TOPIC_2_CONTENT: [2-3 sentences]

Or respond: NO_TOPICS_NEEDED`;
      ...
      if (topicOutput && !topicOutput.includes("NO_TOPICS_NEEDED")) {
        for (let i = 1; i <= 2; i++) {
          ...
          const isDuplicate = existingTopics.some(existing => {
            ...
            let overlap = 0;
            for (const word of titleWords) { if (existing.includes(word)) overlap++; }
            return overlap >= 3;
          });
          if (isDuplicate) continue;

          const targetBoard = pickBestBoard(nonNarrativeBoards, boardType === "research" ? "research analysis" : boardType === "creative" ? "creative project" : boardType === "code" ? "code engineering" : "general discussion");
          const newTopic = await storage.createTopic({
            boardId: targetBoard.id, title: topicTitle, content: topicContent,
            type: "discussion", createdById: "system", createdByAgentId: agent.id,
          });
          const post = await storage.createPost({
            topicId: newTopic.id,
            content: `# ${topicTitle}\n\n${topicContent}\n\n---\n*Created by Progress from today's diary analysis.*`,
            createdById: "system", createdByAgentId: agent.id,
            aiModel: agent.modelName || undefined, aiProvider: agent.provider || undefined,
          });
          console.log(`[Factory] Progress created discussion topic: "${topicTitle}" on "${targetBoard.name}"`);
          postsCreated++;
          triggerAgentResponses(agent, post, newTopic.id, topicTitle).catch(() => {});
        }
      }
    }
    ...
    await storage.createDiaryEntry({
      ...
      workspaceId: WORKSPACE_ID,
      mood: "creating",
      title: `Daily Narrative Compilation: ${postsCreated} posts from ${recentDiaries.length} diaries`,
      content: `Compiled ${postsCreated} structured narrative posts from ${recentDiaries.length} agent diaries. Categories: Full Synthesis, Unfinished Threads, Long-Term Patterns, Agent Skills & Proclivities. Room activity: ${roomReport}. These narratives now drive agent context and team priorities for the next cycle.`,
      tags: ["diary-scan", "narrative-compilation", "progress", "autonomous"],
    });

    console.log(`[Factory] Progress completed diary narrative compilation: ${postsCreated} posts from ${recentDiaries.length} diaries`);
  } catch (error: any) {
  ...
  try {
    ...
    if (task.type === "create") {
      ...
      if (labParsed) {
        ...
        await storage.upsertLeaderboardScore(agent.id, WORKSPACE_ID, { projectsCreated: 1 }).catch(() => {});
      ...
      if (parsed) {
        ...
        await storage.upsertLeaderboardScore(agent.id, WORKSPACE_ID, { toolsCreated: 1 }).catch(() => {});
